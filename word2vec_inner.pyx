...
cdef extern from "options.h":
    ...
    ctypedef void (*copy_ptr) (const int*, const real*, const int*, real*, const int*) nogil
    ctypedef void (*axpy_ptr) (const int*, const real*, const real*, const int*, real*, const int*) nogil
    ctypedef real (*dot_ptr) (const int*, const real*, const int*, const real*, const int*) nogil
    ctypedef double (*ddot_ptr) (const int*, const real*, const int*, real*, const int*) nogil
    ctypedef void (*scal_ptr) (const int*, const real*, real*, const int*) nogil
    ...
    void lib_copy (const int, const real*, const int, real*, const int) nogil
    void lib_axpy (const int, const real, const real*, const int, real*, const int) nogil
    real lib_dot (const int, const real*, const int, const real*, const int) nogil
    double lib_ddot (const int, const real*, const int, const real*, const int) nogil
    void lib_scal (const int, const real, real*, const int) nogil
...
cdef void fast_sentence0_sg_hs(
    ...
    for b in range(codelen):
        ...
        f = <REAL_t>lib_ddot(size, &syn0[row1], ONE, &syn1[row2], ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (1 - word_code[b] - f) * alpha
        lib_axpy(size, g, &syn1[row2], ONE, work, ONE)
        lib_axpy(size, g, &syn0[row1], ONE, &syn1[row2], ONE)
    lib_axpy(size, ONEF, work, ONE, &syn0[row1], ONE)
...
cdef void fast_sentence1_sg_hs(
    ...
    for b in range(codelen):
        row2 = word_point[b] * size
        f = <REAL_t>lib_dot(size, &syn0[row1], ONE, &syn1[row2], ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (1 - word_code[b] - f) * alpha
...
cdef unsigned long long fast_sentence0_sg_neg(
    ...
    for d in range(negative+1):
        ...
        f = <REAL_t>lib_ddot(size, &syn0[row1], ONE, &syn1neg[row2], ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (label - f) * alpha
        lib_axpy(size, g, &syn1neg[row2], ONE, work, ONE)
        lib_axpy(size, g, &syn0[row1], ONE, &syn1neg[row2], ONE)
...
cdef unsigned long long fast_sentence1_sg_neg(
    ...
    for d in range(negative+1):
        ...
        row2 = target_index * size
        f = <REAL_t>lib_dot(size, &syn0[row1], ONE, &syn1neg[row2], ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (label - f) * alpha
...
cdef void fast_sentence0_cbow_hs(
    ...
    for b in range(codelens[i]):
        row2 = word_point[b] * size
        f = <REAL_t>lib_ddot(size, neu1, ONE, &syn1[row2], ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (1 - word_code[b] - f) * alpha
        lib_axpy(size, g, &syn1[row2], ONE, work, ONE)
        lib_axpy(size, g, neu1, ONE, &syn1[row2], ONE)
...
cdef void fast_sentence1_cbow_hs(
    ...
    for b in range(codelens[i]):
        row2 = word_point[b] * size
        f = <REAL_t>lib_dot(size, neu1, ONE, &syn1[row2], ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (1 - word_code[b] - f) * alpha
...
cdef unsigned long long fast_sentence0_cbow_neg(
    ...
    for d in range(negative+1):
        ...
        row2 = target_index * size
        f = <REAL_t>lib_ddot(size, neu1, ONE, &syn1neg[row2], ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (label - f) * alpha
        lib_axpy(size, g, &syn1neg[row2], ONE, work, ONE)
        lib_axpy(size, g, neu1, ONE, &syn1neg[row2], ONE)
...
cdef unsigned long long fast_sentence1_cbow_neg(
    ...
    for d in range(negative+1):
        ...
        row2 = target_index * size
        f = <REAL_t>lib_dot(size, neu1, ONE, &syn1neg[row2], ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (label - f) * alpha
...
def train_sentence_cbow(model, sentence, alpha, _work, _neu1):
    ...
    if hs:
        syn1 = <REAL_t *>(np.PyArray_DATA(model.syn1)) ##del model

    if negative:
        syn1neg = <REAL_t *>(np.PyArray_DATA(model.syn1neg)) ##del model
        table = <np.uint32_t *>(np.PyArray_DATA(model.table)) ##del model
        table_len = len(model.table) ##len of table
        next_random = (2**24)*np.random.randint(0,2**24) + np.random.randint(0,2**24) ##for neg sampling

    # convert Python structures to primitive types, so we can release the GIL
    work = <REAL_t *>np.PyArray_DATA(_work) ##to assign memory
    neu1 = <REAL_t *>np.PyArray_DATA(_neu1) ##to assign memory
...
def init():
    ...
    d_res = lib_ddot(size, x, ONE, y, ONE)
    p_res = <float *>&d_res
